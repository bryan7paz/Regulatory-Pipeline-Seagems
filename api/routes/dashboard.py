...
def metrics(db: Session = Depends(get_db)):
    ...
    by_aplicacao = (
        db.query(models.RegulatoryAnalysis.aplicacao,
                 func.count(models.RegulatoryAnalysis.id))
        .group_by(models.RegulatoryAnalysis.aplicacao)
        .all()
    )
    by_status = (
        db.query(models.RegulatoryAnalysis.status,
                 func.count(models.RegulatoryAnalysis.id))
        .group_by(models.RegulatoryAnalysis.status)
        .all()
    )
    by_validacao = (
        db.query(models.RegulatoryAnalysis.status_validacao,
                 func.count(models.RegulatoryAnalysis.id))
        .group_by(models.RegulatoryAnalysis.status_validacao)
        .all()
    )
    by_assunto = (
        db.query(models.RegulatoryAnalysis.assunto,
                 func.count(models.RegulatoryAnalysis.id))
